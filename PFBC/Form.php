...
class Form extends Base {
	...
	public function render($returnHTML = false) {
		...
		$this->view->_setFieldset($this->fieldset);
    ...
    /*Submit Message Before Form Ends.*/
    public static function renderSubmitMessage() {
        echo '<span class="submit-message" style="display:none;"></span>';
    }
	...
	protected function renderCSSFiles() {
		$urls = array();
		if(in_array("bootstrap_no", $this->prevent))
...
JS;
	...
	protected function renderJSFiles() {
		$urls = array();
		if(in_array("jQuery_no", $this->prevent))
    ...
    /*Set Fieldset*/
    public function setFieldset(array $values) {
        foreach($values as $key => $val) {
            $this->fieldset->setAttribute($key, $val);
        }
    }
}
